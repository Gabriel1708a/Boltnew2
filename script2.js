...
function updateProgressBars() {
  ...
  if (
    localStorage.getItem(`script2_progressBars_index1_${timeKey}`) &&
    localStorage.getItem(`script2_progressBars_index2_${timeKey}`) &&
    localStorage.getItem(`script2_progressBars_index3_${timeKey}`)
  ) return;
  ...
  localStorage.setItem(`script2_progressBars_index1_${timeKey}`, JSON.stringify(data1));
  localStorage.setItem(`script2_progressBars_index2_${timeKey}`, JSON.stringify(data2));
  localStorage.setItem(`script2_progressBars_index3_${timeKey}`, JSON.stringify(data3));
}

function applyStoredPercentages() {
  const timeKey = getCurrentTimeKey();
  const data1 = JSON.parse(localStorage.getItem(`script2_progressBars_index1_${timeKey}`)) || {};
  const data2 = JSON.parse(localStorage.getItem(`script2_progressBars_index2_${timeKey}`)) || {};
  const data3 = JSON.parse(localStorage.getItem(`script2_progressBars_index3_${timeKey}`)) || {};
...
function updateMinPagantesWithTimestamp() {
  ...
  document.querySelectorAll(".square-container").forEach((container, index) => {
    const key = `script2_minPagantes_${index}_${timeKey}`;
...
function iniciarContadorRegressivo() {
  ...
  function atualizarTimer() {
    ...
    if (diffMs <= 1000 && !window._atualizou2) {
      window._atualizou2 = true;

      setTimeout(() => {
        updateProgressBars();
        applyStoredPercentages();
        updateMinPagantesWithTimestamp();
        window._atualizou2 = false;
      }, 1000);
    }
  }
...
function criarLayoutDinamico() {
  const container = document.getElementById("container-pp");
  container.innerHTML = "";
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  fetch("pp.json")
    .then((response) => {
      if (!response.ok) throw new Error("Erro ao carregar pp.json");
    ...
    .catch((error) => console.error("Erro ao carregar jogos PP:", error));

    // === 🟢 Scroll até a seção "Como Jogar" ===
  const scrollBtn = document.getElementById("scrollBtn");
  const comoJogar = document.getElementById("como-jogar");

  if (scrollBtn && comoJogar) {
    scrollBtn.addEventListener("click", () => {
      comoJogar.scrollIntoView({ behavior: "smooth" });
    });
  }
});
